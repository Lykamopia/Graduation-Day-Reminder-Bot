...
#     if days_left > 0:
#         countdown_str = f"<b>Days left:</b> "
#         countdown_str += f"{days_left} days\n"
...
#     progress = (GRADUATION_DATE - datetime.date.today()).days
#     remaining = days_left
#     progress_percent = int((progress / (progress + remaining)) * bar_length)
#     bar = '▓' * progress_percent + '░' * (bar_length - progress_percent)
#     return f"<code>{bar}</code>"

# if __name__ == "__main__":
#     asyncio.run(post_countdown_to_telegram())
...
#             # Sleep for a minute to avoid multiple sends in the same minute
#             time.sleep(60)
